...
class MaskedAutoencoderViT(nn.Module):
    ...
    def forward(self, imgs, mask_ratio=0.75):
        ...
        if not self.training:
            ...
            return cls_token, cls
        pred = self.forward_decoder(latent, ids_restore)  # [N, L, p*p*3]
        cls_token = latent[:, 0, :]  # [N, D]
        cls = self.online_cls(cls_token.detach())
        loss = self.forward_loss(imgs, pred, mask)
        return cls_token, loss, pred, mask, cls
...
def mae_vit_base_patch4_32_dec512d8b(**kwargs):
    model = MaskedAutoencoderViT(
...
mae_vit_base_patch4_32 = mae_vit_base_patch4_32_dec512d8b
