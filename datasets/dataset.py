...
class DatasetGenerator():
    def __init__(self, train_bs=128, eval_bs=256, seed=0, n_workers=4,
        ...
        if kwargs['collate_fn']['name'] == 'None':
            self.collate_fn = None
        else:
            self.collate_fn = collate_fn_options[kwargs['collate_fn']['name']](**kwargs['collate_fn'])
        self.train_set = dataset_options[train_d_type](train_path, train_tf, False, kwargs)
        self.eval_train_set = dataset_options[train_d_type](train_path, test_tf, False, kwargs)
    ...
    def get_loader(self, train_shuffle=True, drop_last=False, train_sampler=None, test_sampler=None):
        if train_shuffle is False or train_sampler is None:
            train_loader = DataLoader(dataset=self.train_set, pin_memory=True,
                                      ...
                                      num_workers=self.n_workers, collate_fn=self.collate_fn,
                                      shuffle=train_shuffle)
            eval_train_loader = DataLoader(dataset=self.eval_train_set, pin_memory=True,
